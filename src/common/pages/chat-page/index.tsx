...
import { formatLastSeen, formatTime, generateMessageKey, getInitial } from '@/shared'
...
export default function ChatPage() {
      ...
      switch (data.type) {
        ...
        case 'private_message':
          if (
            ...
            String(data.receiverId) === String(session?.user?.id) ||
            String(data.senderId) === String(session?.user?.id)
          ) {
            ...
            setMessages(prev => [
              ...prev,
              {
                id: data.id || `msg-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
        ...
        case 'user_joined':
          ...
          if (data.onlineUsers) {
            ...
            setOnlineUsers(prev => {
              const userExists = prev.some(user => String(user.userId) === String(data.senderId))
              if (!userExists) {
                return [
                  ...prev,
                  {
          ...
          if (String(data.senderId) !== String(session?.user?.id)) {
            setMessages(prev => [
              ...prev,
              {
        ...
        case 'typing':
          if (String(data.receiverId) === String(session?.user?.id)) {
            if (data.isTyping) {
  ...
  useEffect(() => {
    ...
    websocket.onclose = () => {
      console.log('Disconnected from chat-page')
      setWs(null)
  ...
  }, [session?.user?.id, session?.user?.name, handleIncomingMessage])
  ...
  const startChatWithUser = (user: OnlineUser) => {
    ...
    console.log('Starting chat-page with user:', user.userId)
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <p className={'text-center'}>{formatTime(msg.timestamp, 'time')}</p>
