...
export default function PostsPage() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      const response = await instance.get(`api/posts/get-user-posts`)
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await instance.post('api/posts/add-post', {
        title: title.trim(),
  ...
  const handleDelete = async (postId: number) => {
    try {
      await instance.delete(`api/posts/delete-post/${postId}`)
